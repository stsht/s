...
function buildClientSummaries(clientRows = [], invoices = [], deliveries = [], subscriptions = [], q = '') {
  ...
  (Array.isArray(subscriptions) ? subscriptions : []).forEach((subscription) => {
    const summary = bucketFor(subscription, 'subscription');
    if (!summary) return;
    summary.subscription_count += 1;
    summary.subscription_ids.push(String(subscription.id));
    if (!summary.contact && subscription.client_contact) summary.contact = cleanText(subscription.client_contact, 240);
  });
